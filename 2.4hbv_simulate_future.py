...
for id in basin_list['id']:
    for bname in basin_list['name']:
        #read  data
        access = pd.read_csv(f'ncss_data/processed/{bname}_ACCESS-CM2_{scenario}.csv')
        cnrm = pd.read_csv(f'ncss_data/processed/{bname}_CNRM-CM6-1_{scenario}.csv')
        ...
        #save output
        df.to_csv(f'output/future_flow/output{id}_{scenario}.csv', index=False)